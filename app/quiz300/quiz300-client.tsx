...
import { IconHome, IconChevronLeft, IconChevronRight, IconFileEdit } from '@/components/icons'
...
export default function Quiz300Client() {
  ...
  useEffect(() => {
    async function checkAccess() {
      ...
      if (!session) { router.replace('/auth?redirect=/quiz300'); return }
  ...
  async function handleProfileContinue() {
    ...
    // Check for existing completed 300 result
    const supabase = createClient()
        ...
        <div className="page-wrap max-w-2xl">
          <section className="glass-panel rounded-[2rem] px-6 py-8 sm:px-10 sm:py-10">
            ...
            <p className="body-soft mt-4 text-base leading-8">
              ต่อจาก 120 ข้อ — ต้องทำเพิ่มอีก <strong>180 ข้อใหม่</strong> (~50–70 นาที)<br />
              ข้อมูลที่กรอกจะใช้ร่วมกับผล 120 ข้อของคุณ
            </p>

            <div className="mt-6 section-panel rounded-[1.5rem] px-5 py-4">
              <p className="text-sm text-[var(--text-soft)] leading-[1.6]">
                ✅ คะแนนจาก <strong>120 ข้อเดิม</strong>จะถูกนำมาใช้โดยอัตโนมัติ ไม่ต้องทำซ้ำ
              </p>
            </div>

            <div className="mt-6 section-panel rounded-[1.75rem] p-5 sm:p-6 space-y-5">
              <div>
                <label htmlFor="age300" className="field-label">อายุ</label>
                <input id="age300" type="number" min={10} max={120} value={age}
              ...
              <div>
                <label htmlFor="occ300" className="field-label">อาชีพ หรือ ความเชี่ยวชาญ</label>
                <input id="occ300" type="text" value={occupation}
            ...
            <div className="mt-6 space-y-3">
              ...
              <Link href="/results120" className="secondary-button w-full text-sm justify-center flex items-center gap-2">
                <IconChevronLeft />
                กลับดูผล 120 ข้อ
              </Link>
            </div>
      ...
      <main className="page-shell flex items-center justify-center">
        <div className="page-wrap max-w-lg">
          <div className="glass-panel rounded-[2rem] px-6 py-10 text-center">
            ...
            <h2 className="text-xl font-semibold text-[var(--text-main)]">คุณมีผลลัพธ์ 300 ข้ออยู่แล้ว</h2>
            <p className="body-soft mt-3 text-sm leading-[1.7]">
              มีผล 300 ข้อที่บันทึกไว้<strong>{existingDate ? ` เมื่อ ${existingDate}` : ''}</strong><br />
            ...
            <div className="mt-8 space-y-3">
              <button
                onClick={() => router.push(`/results300?id=${existingProfileId}`)}
    ...
    <QuizShell
      config={{
        ...
        restoreState: async () => {
          ...
          // Fallback: load from last completed 300 profile (draft deleted after completion)
          ...
          if (completed?.answers && Object.keys(completed.answers as object).length > 0) {
            // Only restore the +180 new answers (not the 120 overlap)
            const allAnswers = completed.answers as Record<number, number>
            const newItemIds = new Set(items300new.map(i => i.id))
            const filteredAnswers: Record<number, number> = {}
            for (const [k, v] of Object.entries(allAnswers)) {
              if (newItemIds.has(Number(k))) filteredAnswers[Number(k)] = v
            }
            return {
              answers: filteredAnswers,
              page: TOTAL_PAGES_300,
              responseTimes: {},
              pageDurations: {},
            }
          }
        ...
        onComplete: async ({ answers, responseTimes, pageDurations }) => {
          ...
          router.push(profileRow?.id ? `/results300?id=${profileRow.id}` : '/results300')
        },
      }}
    />
  )
